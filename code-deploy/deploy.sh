...
export NEO4J_USERNAME=$(aws ssm get-parameters --region us-east-1 --names NEO4J_USERNAME --with-decryption --query Parameters[0].Value | sed 's\"\\g')
export NEO4J_PASSWORD=$(aws ssm get-parameters --region us-east-1 --names NEO4J_PASSWORD --with-decryption --query Parameters[0].Value | sed 's\"\\g')
